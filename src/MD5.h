...
#define _MD5_
	...
	// Define the MD5 Hashing Class
	#define MD5_SET(n) (this->_Block[(n)] = (unsigned long)ptr[(n) * 4] | ((unsigned long)ptr[(n) * 4 + 1] << 8) | ((unsigned long)ptr[(n) * 4 + 2] << 16) | ((unsigned long)ptr[(n) * 4 + 3] << 24))
	...
	class MD5 {
		...
		public:
			...
			void Update(const void* data, uint32_t size) {
				...
				// Update the internal state
				if ((this->_Lo = (_Saved_lo + size) & 0x1fffffff) < _Saved_lo) this->_Hi++;
			...
			void Finalize(uint8_t* _Result) {
				...
				_Result[0] = this->_A; _Result[1] = this->_A >> 8; _Result[2] = this->_A >> 16; _Result[3] = this->_A >> 24;
				_Result[4] = this->_B;_Result[5] = this->_B >> 8; _Result[6] = this->_B >> 16; _Result[7] = this->_B >> 24;
				_Result[8] = this->_C; _Result[9] = this->_C >> 8; _Result[10] = this->_C >> 16; _Result[11] = this->_C >> 24;
				_Result[12] = this->_D; _Result[13] = this->_D >> 8; _Result[14] = this->_D >> 16; _Result[15] = this->_D >> 24;
